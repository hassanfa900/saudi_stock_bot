  ...
  try {
    ...
    for (const row of rows) {
      try {
        ...
        const volInT = await getText(4);
        const volOutT = await getText(5);

        const symbol = (symbolT || 'UNKNOWN').replace(/\s+/g, '');
        const name = nameT || '';
        const price = parseNumber(priceT);
        const changePercent = parseNumber(changeT);
        const volumeIn = parseNumber(volInT);
        const volumeOut = parseNumber(volOutT);
